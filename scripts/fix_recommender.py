...
NOTEBOOK = Path(
    r"C:\Users\david\Downloads\Labs 1 - Pandas (1)\PROJECT\notebooks\01_exploratory_data_analysis.ipynb"
)
...
NEW_RECOMMEND_CELL = {
    ...
    "source": [
        ...
        "    1. Averages the feature vector across ALL d-uplicate rows for the query song\n",
...
for cell in cells[recommend_idx + 1 :]:
    ...
    if (
        cell["cell_type"] == "code"
        and "recommend(" in src
        and "def recommend" not in src
    ):
...
print(
    f"Removing {cells_to_remove_after} test cell(s) after it and replacing with 2 new ones."
)
